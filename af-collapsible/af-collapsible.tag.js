riot.tag2('af-collapsible', '<ul><li each="{header, i in headers}" class="{is-active : (header.isOpen && header.children) || (header.isFocused && !header.children)}"><af-raw class="{header : true, is-focused : header.isFocused}" text="{header.title}" onclick="{parent.toggleHeader}"></af-raw><af-button class="headeraction" if="{header.action}" buttontext="{header.action}" bus="{opts.bus}"></af-button><af-raw class="body" text="{header.body}" if="{header.body}"></af-raw><af-collapsible class="body" headers="{header.children}" if="{header.children}" bus="{parent.opts.bus}" multiple="{parent.opts.multiple}"></af-collapsible></li></ul>', '', '', function(opts) {
  var self = this;
